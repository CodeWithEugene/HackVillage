...
- Money is held by Paystack (licensed payment provider); after deposit it never passes back through the organizer's hands. Failed payouts retry, then go to manual review — funds stay in escrow until a payout is confirmed.
- Every vault lock and payout is attested on a public ledger (Polygon, PrizeVault contract), with references stored as hashes so anyone can verify without exposing personal data.
...
- [Developer profiles](${appUrl("/hackathons")}): every registered builder has a public Proof-of-Work profile at /developers/:handle.
